...
--
-- Reemplazar el UUID de :rid por el del local a revisar.
-- ════════════════════════════════════════════════════════════════════

\set rid '7f5e4829-33c9-46e4-ae3a-3f4bfed0ad7b'
...
SELECT id, name, delivery_config -> 'prep_policy' AS prep_policy
  FROM restaurants
 WHERE id = :'rid';

-- 3) Estaciones del local y su alcance real.
...
--                                       esos ítems.
--    Antes del fix, cualquier estación con categorías vaciaba la central:
--    un local con estaciones por rubro (Cocina, Bebidas, Postres…) se quedaba
--    sin ningún pedido en la pantalla general.
SELECT s.id, s.name, s.is_active,
       s.restaurant_id,                     -- si viene NULL, el link ?station= no resuelve el local
 ...
 GROUP BY s.id, s.name, s.is_active, s.restaurant_id
 ORDER BY s.name;
...
SELECT o.order_number, oi.item_name, mc.name AS categoria, t.zona AS zona_mesa,
       (SELECT string_agg(s2.name, ', ')
         ...
         WHERE s2.restaurant_id = :'rid' AND s2.is_active
...
-- 5) Zonas realmente usadas por las mesas del local. Una estación acotada a una
--    zona que ninguna mesa tiene nunca reclama nada (y eso está bien).
